...
class FirestoreDb extends GetxController {
  ...
  static Future<List<Map<String, dynamic>>> getCategory() {
    ...
    return firebaseFirestore.collection('Categories').get().then((query) {
  ...
  static Future<Map<String, dynamic>?> getUserProfile(String userUi) async {
    ...
    await firebaseFirestore.collection('users').doc(userUi).get().then((value) {
      //  Map<String,dynamic>?  userdoc = value.data();
      data = value.data();
    });
    print("final user categories data ${data}");
  ...
  static Future<List<Map<String, dynamic>>?> getLocations() async {
    ...
    await firebaseFirestore.collection("users").get().then((value) {
      value.docs.forEach((doc) {
        Locationdata.add({'location': doc['location'], 'uid': doc['uid']});
      });
    });
  ...
  static Future<List<Map<String, dynamic>>?> getLocationsfilter(
      ...
      value.docs.forEach((doc) {
        ...
        labels.forEach((element) {
          if (doc['categories'].contains(element)) {
            print(
                "Ca passsssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssss");
            Locationdata.add({'location': doc['location'], 'uid': doc['uid']});
          }
        });
      });
  ...
  static Future<List<Map<String, dynamic>>?> getLocationsfiltersearch(
      String labels) async {
    List<Map<String, dynamic>> Locationdata = [];
    await firebaseFirestore.collection("users").get().then((value) {
      value.docs.forEach((doc) {
        if (doc['categories'].contains(labels) ||
            doc['name'].contains(labels)) {
          print(
              "Ca passsssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssss");
          Locationdata.add({'location': doc['location'], 'uid': doc['uid']});
        }
  ...
  static UpdateUserProfil(String tel, name) async {
    await firebaseFirestore
        ...
        .doc(auth.currentUser!.uid)
        .update(
      {'telephone': tel, 'name': name},
    );
    if (auth.currentUser!.displayName == null || auth.currentUser != name) {
